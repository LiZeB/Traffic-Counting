...
class VehicleCounter(PipelineProcessor):
    ...
    def __call__(self, context):
        ...
        if not self.pathes:  # not []表达式的值是True
            for match in points:
            ...
            for path in self.pathes:  # 这里的self.pathes保存了之前帧的path
                _min = 999999
                _match = None
                for p in points:
                    if len(path) == 1:  # 如果之前帧的path只有一个点
                        ...
                        # 这里的xn和yn是基于之前帧的两个点评估下一帧同一个检测目标应该出现的位置
                        # 后面再根据实际帧中检测目标出现的实际位置与评估位置的距离
                    ...
                    # 后面帧中的检测目标坐标移动的相对位置一定比前面帧中检测目标移动的位置小（近大远小的原理）
                    if d < _min:
                        _min = d
                        _match = p
